// Implementación REAL de Image360DDSPublisher. Compilada solo cuando esta
// build tiene FastDDS (ver ../CMakeLists.txt).

#include "image360_channel.h"
#include "media_transport.h"

#include <cstring>

struct Image360DDSPublisher::Impl
{
    DDSChannel::Config          cfg;
    rc::media::Image360Publisher pub;
    bool                         ready    = false;
    std::uint64_t                frame_id = 0;
};

Image360DDSPublisher::Image360DDSPublisher(Config cfg) : pimpl_(std::make_unique<Impl>())
{
    pimpl_->cfg = std::move(cfg);
}

Image360DDSPublisher::~Image360DDSPublisher() = default;

bool Image360DDSPublisher::init()
{
    rc::media::PublisherConfig pc;
    pc.domain_id          = pimpl_->cfg.domain_id;
    pc.topic_name         = pimpl_->cfg.topic;
    pc.history_depth      = pimpl_->cfg.history_depth;
    pc.shared_memory_only = pimpl_->cfg.shared_memory_only;
    pc.data_sharing       = pimpl_->cfg.data_sharing;
    pimpl_->ready = pimpl_->pub.init(pc);
    return pimpl_->ready;
}

bool Image360DDSPublisher::ready() const { return pimpl_->ready; }

std::string_view Image360DDSPublisher::topic() const { return pimpl_->cfg.topic; }

bool Image360DDSPublisher::publish(const DDSChannel::Frame& base)
{
    if (not pimpl_->ready) return false;
    const auto* f = dynamic_cast<const Frame*>(&base);
    if (f == nullptr || f->rgb == nullptr) return false;

    const std::uint32_t bytes = f->width * f->height * 3;
    if (bytes == 0 || bytes > rc::media::MAX_IMAGE360_BYTES) return false;

    rc::media::Image360Frame* loaned = pimpl_->pub.loan();
    if (loaned == nullptr) return false;

    loaned->frame_id(++pimpl_->frame_id);
    loaned->stamp_ms(f->stamp_ms);
    loaned->stream_id(rc::media::STREAM_RICOH_RGB);
    loaned->width(f->width);
    loaned->height(f->height);
    loaned->step(f->width * 3);
    loaned->format(rc::media::IMG360_FORMAT_RGB8);
    loaned->size(bytes);
    auto& buf = loaned->data();
    std::memcpy(buf.data(), f->rgb, bytes);

    if (pimpl_->pub.publish(loaned)) return true;
    pimpl_->pub.discard(loaned);
    return false;
}

DDSOptional make_image360_channel(DDSChannel::Config cfg)
{
    auto ch = std::make_unique<Image360DDSPublisher>(std::move(cfg));
    if (not ch->init()) return {};
    return DDSOptional{std::move(ch)};
}
